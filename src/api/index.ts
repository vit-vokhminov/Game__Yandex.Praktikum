...
export const API = {
    ...
    async getLeaders() {
        return await instanceAPI.get(`/api/leaders`);
    },

    // добавить лидера
    async addLeader(user: any) {
        return await instanceAPI.post(`/api/leader`, user);
    },

    // Изменить рекорд юзера
    async editRecordLeader(user: any) {
        return await instanceAPI.put(`/api/leader`, user);
    },
};
